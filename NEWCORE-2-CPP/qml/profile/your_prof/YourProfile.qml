...
// K-Server connection that doesn't exist yet, so it shows a genuine
// "no internet connection" state instead of a made-up upload prompt.
...
Item {
    ...
    ColumnLayout {
        ...
        Item {
            ...
            // Decorative animated rainbow ring, opt-in (see
            // AppSettings::profileGradientEnabled). It's sized a bit
            // larger than avatarFrame and sits *behind* it in paint
            // order (declared first, and avatarFrame is fully opaque),
            // so only its outer edge peeks out as a spinning ring —
            // no separate circular mask needed.
            Rectangle {
                id: gradientRing
                visible: appSettings.profileGradientEnabled
                width: Kirigami.Units.gridUnit * 6.5 + 12
                height: width
                radius: width / 2
                anchors.left: parent.left
                anchors.leftMargin: Kirigami.Units.largeSpacing - 6
                anchors.bottom: parent.bottom
                anchors.bottomMargin: -(Kirigami.Units.gridUnit * 6.5 * 0.35) - 6
                gradient: Gradient {
                    GradientStop { position: 0.0; color: "#ff0080" }
                    GradientStop { position: 0.16; color: "#ff8c00" }
                    GradientStop { position: 0.33; color: "#ffee00" }
                    GradientStop { position: 0.5; color: "#00ff88" }
                    GradientStop { position: 0.66; color: "#00c8ff" }
                    GradientStop { position: 0.83; color: "#8000ff" }
                    GradientStop { position: 1.0; color: "#ff0080" }
                }
                RotationAnimation on rotation {
                    running: appSettings.profileGradientEnabled
        ...
        RowLayout {
            ...
            ToolButton {
                icon.name: appSettings.profileGradientEnabled ? "color-picker" : "color-management"
                checkable: true
                checked: appSettings.profileGradientEnabled
                onClicked: appSettings.profileGradientEnabled = !appSettings.profileGradientEnabled
                ToolTip.visible: hovered
                ToolTip.text: root.tr("profile.gradient_toggle")
            }
            ToolButton {
        ...
        // ── Body: about + media tabs (left) | friends (right) ──
        RowLayout {
            Layout.fillWidth: true
