...
def _get_sound_dirs() -> list:
    ...
    Search order (most specific → fallback):
      ...
      6. /usr/share//sounds — Linux system install
      7. %APPDATA%//sounds — Windows fallback
    """
    dirs = []
    ...
    dirs.append(Path("/usr/local/share//sounds"))
    # 7) Windows %APPDATA%
    try:
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            dirs.append(Path(appdata) / "" / "sounds")
    except Exception:
        pass
    return dirs
...
# ═══════════════════════════════════════════════════════════════════════════
# ─────────────────────────────────────────────────────────────────────────────
#  MEDIA OPEN HELPER — always ask:  or system default
# ─────────────────────────────────────────────────────────────────────────────
_MEDIA_PREF: str = ""   # "" | "system" | "" (ask each time)
...
def _open_media_smart(path: str, parent=None):
    """Open a media file: ask whether to use  or system default.
    ...
    btn_ = QPushButton(TR("player_builtin"))
    btn_.setObjectName("accent_btn"); btn_.setFixedHeight(34)
    ...
    btn_.clicked.connect(lambda: _pick(""))
    btn_sys.clicked.connect(lambda: _pick("system"))
    btn_row.addWidget(btn_); btn_row.addWidget(btn_sys)
    vl.addLayout(btn_row)
    dlg.exec()

    if _choice[0] == "":
        _open_in_(path, parent)
    elif _choice[0] == "system":
        _open_system(path)
...
def _open_in_(path: str, parent=None):
    """Add file to , switch to  tab, and start playback."""
    # Search all top-level windows for MainWindow (has __player)
    app_inst = QApplication.instance()
    if app_inst:
        for w in app_inst.topLevelWidgets():
            if hasattr(w, '__player') and hasattr(w, '_tabs'):
                mp = w.__player
                # Switch to  tab first
    ...
    for _ in range(12):
        if w is None: break
        if hasattr(w, '__player') and hasattr(w, '_tabs'):
            w._tabs.setCurrentWidget(w.__player)
            w.__player.open_file(path)
            return
...
# ─────────────────────────────────────────────────────────────────────────────
#  MEDIA OPEN HELPER — спрашивает открыть через  или системой
...
class NotificationCenter:
    """
    Централизованная система внутренних уведомлений .
...
THEMES = {
    ...
    "win95": {
        # ──  1.7543 — Windows 95 Easter Egg theme ──────────────
        # Activate via: Настройки → Темы → 1.7543
        # The name "1.7543" is the previous version number of .
        "label": "1.7543",
...
class AppSettings:
    ...
    def connection_mode(self) -> str:
        """'lan' — LAN/VPN only, 'internet' — через VDS .ru"""
...
def _show_update_dialog(ver: str, desc: str, parent=None):
    ...
    -Windows-x86_64.exe и -Linux-x86_64.AppImage.
    """
    ...
    # Заголовок
    title = QLabel(f' v{ver} ' + _L("доступна!", "available!", "利用可能!"))
    title.setStyleSheet(
    ...
    for label, url, is_current in [
        ("⬇ Windows .exe",
         f"{_base}/-Windows-x86_64.exe",
         is_win),
        ("⬇ Linux AppImage",
         f"{_base}/-Linux-x86_64.AppImage",
         is_linux),
        ("📦 Исходники",
         f"{_release_page}",
